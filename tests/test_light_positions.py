...
def test_z_is_cosine_of_elevation():
    for (theta, _phi), (_x, _y, z) in zip(POLAR_LIGHT_LIST, light_vectors()):
...
def test_adjustment_leaves_elevation_untouched():
    for (_x, _y, z0), (_x2, _y2, z1) in zip(light_vectors(0), light_vectors(37)):
        assert math.isclose(z0, z1, abs_tol=1e-9)


def test_adjustment_rotates_about_the_vertical_axis():
    for (x0, y0, _z0), (x1, y1, _z1) in zip(light_vectors(0), light_vectors(90)):
...
def test_full_turn_is_identity():
    for base, turned in zip(light_vectors(0), light_vectors(360)):
